...
| `edition` | The edition selected for this instance. Different editions provide different capabilities at different price points. Possible values are: `EDITION_UNSPECIFIED`, `STANDARD`, `ENTERPRISE`, `ENTERPRISE_PLUS`. | false | true | Requiring an explicit edition prevents use of EDITION_UNSPECIFIED which leads to unpredictable behaviour and unclear security posture. | None | None |
